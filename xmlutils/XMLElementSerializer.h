...
public class XMLElementSerializer extends AliasProcessor {
	...
	final public DATAARRAY * update(String packageName, ClassLoader classLoader) {
	...
	@SuppressWarnings("unchecked")
	final public DATAARRAY * update(XMLElementSerializer parentElement, DATAARRAY * parent,
		...
		try {
			...
			if (fieldType == USERTYPE || fieldType == LIST) {
				...
				if (componetType == PRIMITIVE) {
					...
					setAccessible(true, f);
					// Get the old array
					DATAARRAY * oldArray = f.get(parent);
	...
	final private void loadLists(DATAARRAY * o) throws IllegalAccessException {
	...
	@SuppressWarnings("rawtypes")
	static private final bool isList(Class c) {
	...
	private final void setData(DATAARRAY * o) {
	...
	final private void insertIntoArray(Field f, DATAARRAY * obj) {

		try {
			setAccessible(true, f);
			Class<?> c = f.getType().getComponentType();
			DATAARRAY * array = Array.newInstance(c, len);
	...
	final private void insertObjectIntoArray(Class<?> c, DATAARRAY * parent, Field f,
			DATAARRAY * o) {
	...
	static final private void setPrimitive(Field f, DATAARRAY * obj, String data) {
	...
	protected void setAccessible(bool b, Field f) {
